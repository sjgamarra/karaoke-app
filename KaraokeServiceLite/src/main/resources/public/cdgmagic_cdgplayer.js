...
function set_file_prefix(rutaArchivo)
{
	if (rutaArchivo == "")  { return; };
	
	var audio_file_prefix = rutaArchivo;
    var cdg_file_prefix = rutaArchivo;
...
function xml_status_handler()
{
    ...
    if (xml_file_request.readyState == 4)
    {
    	...
    	//When requesting local files need to also look for status 0
//        if (xml_file_request.status === 200 || xml_file_request.status == 0)
//        {
//        	cdg_file_data = xml_file_request.responseText;
//        }else{
//        	status_text = "Error: HTTP " + xml_file_request.status + "/" + xml_file_request.statusText + " (No graphics!)";
...
function cdgPlayer_reproducirSiguiente(){
	...
	//cambiar estado de pedido actual a EN_REPRODUCCION
